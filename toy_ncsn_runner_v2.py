...
class JointLearningModel(nn.Module):
    def __init__(self, input_dim, num_classes, data_classes):
        super().__init__()
        self.lin1 = ConditionalLinear(input_dim, 32, num_classes, data_classes)
        self.lin2 = ConditionalLinear(32, 32, num_classes, data_classes)
        # self.lin3 = nn.Linear(128, input_dim)
        self.lin4 = nn.Linear(32, data_classes)
        self.lin5 = nn.Linear(data_classes, input_dim)
    
    def forward(self, x, s, c):
        x = F.softplus(self.lin1(x, s, c))
        x = F.softplus(self.lin2(x, s, c))
        logits = self.lin4(x)
        score = self.lin5(logits)
        return score, logits
...
def get_model(config):
    ...
    elif config.training.mode == 'clf-guid':
        gen_model = ConditionalModel(config.input_dim,config.model.num_classes)
        clf_model = ClassifierModel(config.input_dim,config.model.num_classes, 2)
        return gen_model, clf_model
...
def anneal_dsm(scorenet, samples, sigmas, classes=None, anneal_power=2.0):
    ...
    if classes == None: # For unconditional
        scores, _ = scorenet(perturbed_samples, labels, None)
...
class NCSNRunner():
    ...
    def joint_learning_train_sl(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            # Freeze 
            if False:
                update_param_names = ['lin4.weight','lin4.bias']
                for name, param in score.named_parameters():
                    if name in update_param_names:
                        param.requires_grad = True
                        print(f'requires_grad[True]:{name}')
                    else:
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                # class conditional
                dsm_cond, p_samples_cond, scores_cond = anneal_dsm(score, X, sigmas, classes=y, anneal_power=self.config.training.anneal_power)
                # class unconditional
                dsm_uncond, p_samples_uncond, scores_uncond = anneal_dsm(score, X, sigmas, classes=None, anneal_power=self.config.training.anneal_power)
                # classification
                ce_loss, p_samples_clf, acc = anneal_ce_loss(score, X, sigmas, y)
                # Total LOSS
                L += dsm_cond*0.5 + dsm_uncond*0.5 + ce_loss
                ...
                if False:
                    ...
                    with torch.enable_grad():
                        ...
                        _, x_logits = score(x_samples, x_labels, None)
            ...
            if epoch % 100 == 0:
                ...
                for i, (test_X, test_y) in enumerate(test_loader):
                    ...
                    with torch.no_grad():
                        ...
                        _, t_logits = test_score(test_X, None, None)
                ...
                if self.config.training.mode == 'joint_learning' and epoch % 100 == 0:
                    ...
                    _, all_logits = score(all_data, all_labels, None)
                    ...
                    with torch.enable_grad():
                        ...
                        _, s_logits = score(xx_in, xx_labels, None)
                        log_probs = F.log_softmax(s_logits, dim=-1)
                        # class 0's log_probs
                        selected_0 = log_probs[range(len(s_logits)), 0]
                        grad_probs_0 = torch.autograd.grad(selected_0.sum(), xx_in)[0]
                    plot_scores(all_data.T.cpu(), xx.T.cpu(), grad_probs_0.T.cpu(),
                                os.path.join(self.args.plt_fifure_path, f"grad_probs/grad_p(y=0|x)_{epoch}.png"), plot_lim=xy_lim, posterior=all_post)
                    ...
                    plot_scores(all_data.T.cpu(), xx.T.cpu(), scores_log1p_c0.T.cpu(),
                                os.path.join(self.args.plt_fifure_path, f"cond_scores/scoreslog_p(x|y=0)_{epoch}.png"), plot_lim=xy_lim, posterior=all_post)
                    ...
                    plot_scores(all_data.T.cpu(), xx.T.cpu(), scores_log1p_c1.T.cpu(),
                                os.path.join(self.args.plt_fifure_path, f"cond_scores/scoreslog_p(x|y=1)_{epoch}.png"), plot_lim=xy_lim, posterior=all_post)

                    # # ベクトルの足し算したものを表示する
                    hikizan_c0 = (grad_probs_0.cpu()*0.5)+scores_log1p
                    plot_scores(all_data.T.cpu(), xx.T.cpu(), hikizan_c0.T,
                                os.path.join(self.args.plt_fifure_path, f"CG_theory/0_p(y=0|x)+p(x)_{epoch}.png"), plot_lim=xy_lim, posterior=all_post)
                    hikizan_c0 = scores_log1p_c0-scores_log1p
                    plot_scores(all_data.T.cpu(), xx.T.cpu(), hikizan_c0.T,
                                os.path.join(self.args.plt_fifure_path, f"CG_theory/0_p(x|y=0)-p(x)_{epoch}.png"), plot_lim=xy_lim, posterior=all_post)
                    #                     # ベクトルの足し算したものを表示する
    ...
    def joint_learning_train_ssl(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            # Freeze
            if False:
                # update_param_names = ['lin1.lin.weight','lin1.lin.bias','lin1.embed_scale.weight','lin1.embed_class.weight','lin2.lin.weight','lin2.lin.bias','lin2.embed_scale.weight','lin2.embed_class.weight','lin4.weight','lin4.bias']
                # enc_param_names = ['lin1.lin.weight','lin1.lin.bias','lin1.embed_scale.weight','lin1.embed_class.weight','lin2.lin.weight','lin2.lin.bias','lin2.embed_scale.weight','lin2.embed_class.weight']
                enc_param_names = ['lin1.lin.weight','lin1.lin.bias','lin1.embed_scale.weight','lin1.embed_class.weight']
                fc_param_names = ['lin4.weight','lin4.bias']
                update_param_names = fc_param_names + enc_param_names
                print(update_param_names)
                for name, param in score.named_parameters():
                    if name in update_param_names:
                        param.requires_grad = True
                        print(f'requires_grad[True]:{name}')
            ...
            for i, (X, y) in enumerate(train_loader):
                ...
                # dsm_loss_uncond, _, _, uncond_scores = anneal_dsm(test_score, X, sigmas, classes=None, anneal_power=self.config.training.anneal_power)
                dsm_loss_uncond, _, _ = anneal_dsm(score, X, sigmas, classes=None, anneal_power=self.config.training.anneal_power)
                L += dsm_loss_uncond*1.0
                ce_loss, p_samples_clf, acc = anneal_ce_loss(score, labeled_X, sigmas, labeled_y)
                # classification loss
                ...
                dsm_losses.append(dsm_loss_uncond.item())
                ce_losses.append(ce_loss.item())
                total_losses.append(L.item())
                train_accs.append(acc.item())
